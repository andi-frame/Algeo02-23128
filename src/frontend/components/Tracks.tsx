...
const Tracks = () => {
  ...
  useEffect(() => {
    const filteredTracks = tracks
      ? tracks.filter((track) => track.track_name.toLowerCase().includes(searchQuery.toLowerCase()))
      : [];
    ...
    <div className="w-full flex flex-col justify-center items-center text-white cursor-pointer">
      ...
      <div className="w-2/3 mb-5">
        <input
          ...
          className="w-full p-2 rounded-lg ring-spotify-green ring-1 text-white"
        />
      </div>
      <div className="w-2/3 justify-center items-center flex gap-5 flex-col">
        {paginatedTracks.length > 0 ? (
          paginatedTracks.map((track, index) => <TrackCard key={index} track={track} />)
        ) : (
